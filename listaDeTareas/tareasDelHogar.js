...
//Modelo
const Tarea = new mongoose.model("tareasDelHogar", tareaSchema);

//CRUD
//LEER TAREAS
app.get(`/tareasDelHogar`, async (req, res) => {
  const tareasConsulta = await Tarea.find();
  res.json(tareasConsulta);
});

//CREAR TAREA
app.post("/tareasDelHogar", async (req, res) => {
  const {
    titulo,
    descripcion,
    fechaDeCreacion,
...
//ACTUALIZAR TAREA
app.put("/tareasDelHogar/:id", async (req, res) => {
...
//ELIMINAR TAREA
app.delete("/tareasDelHogar/:id", async (req, res) => {
  const idTarea = req.params.id;
